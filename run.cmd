...
:: Config generator
    ...
    echo ##################>> config.txt
    echo # ПРИМЕР КОНФИГА #>> config.txt
    echo ##################>> config.txt
    echo name=Прорыв пердака  *необязательно для Самары>> config.txt
    echo email=email@dosmartec.ru  *необязательно для Самары>> config.txt
    echo source=\\ws100500\d\users\md100500\work\test_project_files\dp0\SYS-1\MECH\>> config.txt
    echo comment=комментарий>> config.txt
